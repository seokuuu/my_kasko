...
export const tableHiddenColumnAtom = atom(
  (get) => {
    ...
    const hiddenIdObj = Object.keys(saved).reduce((acc, key) => ({...acc, [key]: saved[key].hiddenIds || []}), {});
    return hiddenIdObj;
  },
  (get, set, update) => {
 ...
 */
export const tableRestoreColumnAtom = atom(
  null, 
  (get, set, update) => {
    const prevState = get(talbeHiddenAtom);
    const { type } = update;
 ...
 */
export const tableResetColumnAtom = atom(
  null,
  (get, set, update) => {
    set(
      talbeHiddenAtom, 
