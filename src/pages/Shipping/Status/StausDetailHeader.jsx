...
const StausDetailHeader = ({ data }) => {
	...
	const getCustomer = () => {
		...
		// 도착지 목적지 추가
		const customerDestinationPhoneDuplicate = [...new Set(data.map((item) => item.customerDestinationPhone))]
		while (customerDestinationPhoneDuplicate.length < 3) {
			customerDestinationPhoneDuplicate.push('-')
		}
		setCustomerDestinationPhones(customerDestinationPhoneDuplicate)
	}
		...
		data && (
			<TableWrap style={{ marginTop: '5px' }}>
				<ClaimTable>
					...
					<ClaimRow>
						{customerDestinationPhones?.map((customerDestinationPhone, i) => (
							<React.Fragment key={i}>
								<ClaimTitle>도착지 연락처 {i + 1}</ClaimTitle>
								<ClaimContent>
									<div style={{ display: 'flex', marginLeft: '5px' }}>
										<p>{customerDestinationPhone ?? '-'}</p>
									</div>
								</ClaimContent>
							</React.Fragment>
						))}
...
export default StausDetailHeader
