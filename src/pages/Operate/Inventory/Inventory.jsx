...
const Inventory = ({}) => {
  ...
  const [check3, setCheck3] = useState(
    Array.from({ length: checkStores.length }, () => false)
  );
  const [check4, setCheck4] = useState(
    Array.from({ length: checkTransits.length }, () => false)
  );
  const [check5, setCheck5] = useState(
    Array.from({ length: checkShipments.length }, () => false)
  );
  ...
  const [checkData3, setCheckData3] = useState(
    Array.from({ length: checkStores.length }, () => '')
  );

  const [checkData4, setCheckData4] = useState(
    Array.from({ length: checkTransits.length }, () => '')
  );
  const [checkData5, setCheckData5] = useState(
    Array.from({ length: checkShipments.length }, () => '')
  );
                    ...
                    {checkTransits.map((x, index) => (
                      <ExCheckDiv style={{ marginRight: '10px' }}>
                        <StyledCheckSubSquDiv
                          onClick={() =>
                            setCheck4(
                              CheckBox(check4, check4.length, index, true)
                            )
                          }
                    ...
                    {checkShipments.map((x, index) => (
                      <ExCheckDiv>
                        <StyledCheckSubSquDiv
                          onClick={() =>
                            setCheck5(
                              CheckBox(check5, check5.length, index, true)
                            )
                          }
